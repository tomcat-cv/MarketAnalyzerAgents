...
class EvidenceTests(unittest.TestCase):
    ...
    def test_builds_deterministic_brief_around_model_summary(self) -> None:
        ...
        self.assertIn("2026-06-04T16:00:00+08:00", brief)
        self.assertIn("未被采集器捕获不代表事件没有发生", brief)
        self.assertIn("Title only", brief)
        self.assertIn("A股公告标题复核队列", brief)
        self.assertIn("SEC EDGAR", brief)
        self.assertIn("已查询，本窗口无条目", brief)
